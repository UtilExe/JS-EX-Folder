...
// My own note: Atm this returns the whole friend object (including the hashed password). Maybe we want to create a DTO instead, to only respond with certain fields?
...
router.put('/editme', async function (req: any, res, next) {
  try {
    ...
    const friend = await facade.editFriend(email, updatedFriend)
    res.json(friend);
...
router.get("/me", async (req: any, res, next) => {
  try {
    ...
    const emailAddress = req.credentials.userName //GET THE USERS EMAIL FROM SOMEWHERE (req.params OR req.credentials.userName)
    const friend = await facade.getFrind(emailAddress);
    if(friend == null) { // this prolly not needed, cant happen due to verifying on L77 first.
...
router.delete('/:email', async function (req: any, res, next) {
  ...
  try {
    ...
    const email = req.params.email //GET THE USERS EMAIL FROM SOMEWHERE (req.params OR req.credentials.userName)
    const friend = await facade.deleteFriend(email)
    res.json(friend)
