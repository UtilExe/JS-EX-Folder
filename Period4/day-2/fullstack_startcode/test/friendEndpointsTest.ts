...
describe("### Describe the Friend Endpoints (/api/friends) ###", function () {
  ...
  describe("While attempting to add a user", function () {
    ...
    it("It should fail to Add user due to wrong password length", async () => {
      const newFriend = { firstName: "Henrik", lastName: "Jensen", email: "henrik@jan.dk", password: "a"}
      const response = await request.post('/api/friends').send(newFriend);
  ...
  describe("While logged in as a user", function () {
    it("It should return the logged in user", async () => {
      ...
      .get('/api/friends/me')
      .auth("pp@b.dk", "secret")
      expect(response.body.email).equal("pp@b.dk")
    })
    it("It should edit the logged in user", async () => {
      const editedLastName = { firstName: "Peter", lastName: "Panda", email: "pp@b.dk", password: "secret"}
      const response = await request
      .put('/api/friends/editme').send(editedLastName)
      .auth("pp@b.dk", "secret")
      expect(response.status).to.equal(200)
      ...
      .get('/api/friends/me')
      .auth("pp@b.dk", "secret")
      expect(responseMe.body.lastName).to.equal("Panda")
    })
  })
  ...
  describe("While verifying the get any user, given a userId (email)", function () {
    ...
    it("It should not allow admin-users to find a non-existing user", async () => {
      const response = await request
      .get('/api/friends/find-user/notexisting@gmail.com')
      .auth("aa@a.dk", "secret")
  ...
  describe("While verifying the 'edit any user', given a userId (email)", function () {
    it("It should allow an admin-user to edit Peter Pan", async () => {
      const editedPeterPan = { firstName: "PeterG", lastName: "PandaNumpy", email: "pp@b.dk", password: "secret" }
      const response = await request
      .put('/api/friends/pp@b.dk').send(editedPeterPan)
      .auth("aa@a.dk", "secret")
      expect(response.status).to.equal(200)
      ...
      .auth("aa@a.dk", "secret")
      expect(responseMe.body.lastName).to.equal("PandaNumpy")
    })
    it("It should NOT allow a non-admin user to edit Peter Pan", async () => {
      const shouldNotWork = { firstName: "PeterG", lastName: "PandaNumpy", email: "pp@b.dk", password: "secret" }
      const response = await request
      .put('/api/friends/pp@b.dk').send(shouldNotWork)
      .auth("dd@b.dk", "secret")
      expect(response.status).to.equal(401)
    })
  })
  ...
  describe("While verifying the delete any user, given a userId (email)", function () {
    it("It should allow an admin user to delete Donald Duck", async () => {
      ...
      // another way: expect(response.status).to.equal(200)
    })
